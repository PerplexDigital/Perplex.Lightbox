...
class Lightbox {
    constructor(elem, options) {
        ...
        this.animationProps = {
            ...
            item: {
                ...
                staticEaseOpen: this.options.staticEaseOpen
                    ? this.options.staticEaseOpen
                    : 'cubic-bezier(0.21, 0.49, 0.1, 0.99)',
    ...
    open() {
        ...
        if (state.animFrameworks.gsap) {
            state.runningAnimations.container = gsap.fromTo(
                this.element,
                this.animationProps.duration / 1000,
                { opacity: 0, display: 'block' },
                { opacity: 1 }
            );
            state.runningAnimations.item = gsap.fromTo(
                this.item,
                this.animationProps.duration / 1000,
                { scale: 0.9, opacity: 0 },
                {
            ...
            this.element.style.animation = `containerShow ${this.animationProps.duration / 1000
                }s forwards`;
            this.item.style.animation = `itemShow ${this.animationProps.duration / 1000}s ${this.animationProps.item.staticEaseOpen
                } forwards`;
        }
    }
    ...
    close() {
        ...
        if (state.animFrameworks.gsap) {
            ...
            state.runningAnimations.container = gsap.to(
                this.element,
                this.animationProps.duration / 1500,
            ...
            this.element.style.animation = `containerHide ${this.animationProps.duration / 1500
                }s forwards`;
            this.item.style.animation = `itemHide ${this.animationProps.duration / 1500}s ${this.animationProps.item.staticEaseClose
                } forwards`;
        }
    }
    ...
    addCallback(callback, moment) {
        ...
        if (
            ...
            throw new Error(
                "moment must be either 'onOpenBegin', 'onOpenEnd', 'onCloseBegin', 'onCloseEnd'"
            );
        }
